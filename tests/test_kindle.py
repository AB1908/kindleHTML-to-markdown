...
def test_create_highlight_from_div(test_note_divs, example_highlight, highlight_index):
    actual_highlight = kindle.KindleAnnotations().create_text_highlight(test_note_divs[highlight_index], test_note_divs[highlight_index+1])
    assert example_highlight() == actual_highlight

def test_create_new_note_from_div(test_note_divs, example_note, note_index):
    actual_note = kindle.KindleAnnotations().create_new_note(test_note_divs[note_index], test_note_divs[note_index+1])
